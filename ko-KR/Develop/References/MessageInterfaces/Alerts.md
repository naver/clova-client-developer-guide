...
| [`DeleteAlert`](#DeleteAlert)                   | Directive | CLOVA가 특정 알람을 삭제하도록 클라이언트에 지시합니다. |
...
| [`SetAlert`](#SetAlert)                         | Directive | CLOVA가 알람을 새로 추가하거나 특정 알람을 수정하도록 클라이언트에 지시합니다. |
...
| [`StopAlert`](#StopAlert)                       | Directive | CLOVA가 특정 알람을 중지하도록 클라이언트에 지시합니다.  |
| [`SynchronizeAlert`](#SynchronizeAlert)         | Directive | CLOVA가 `payload`에 있는 사용자의 알람 데이터를 동기화하도록 클라이언트에 지시합니다.  |
...
* 클라이언트가 네트워크 접속 장애와 같은 이유로 CIC에 다시 연결될 때
...
CLOVA가 특정 알람을 삭제하도록 클라이언트에 지시합니다. 클라이언트는 특정 알람을 삭제해야 하며, 다음과 같은 이벤트 메시지를 사용하여 그 결과를 CIC에 보고해야 합니다.
...
CLOVA가 알람을 새로 추가하거나 특정 알람을 수정하도록 클라이언트에 지시합니다. 클라이언트는 다음과 같이 알람 추가나 알람 수정을 수행할 수 있습니다.
...
CLOVA가 특정 알람을 중지하도록 클라이언트에 지시합니다. 클라이언트는 지정된 알람을 중지해야 하며, [`AlertStopped`](#AlertStopped) 이벤트 메시지를 사용하여 그 결과를 CIC에 보고해야 합니다.
...
CLOVA가 `payload`에 있는 사용자의 알람 데이터를 동기화하도록 클라이언트에 지시합니다. 클라이언트는 CIC로부터 전달된 데이터에 맞게 클라이언트에 설정된 알람 값을 변경해야 합니다.
