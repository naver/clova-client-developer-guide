...
| [`BtConnect`](#BtConnect)                 | Directive | CLOVA가 특정 블루투스 기기와 연결을 설정하도록 클라이언트에 지시합니다.                               |
| [`BtConnectByPINCode`](#BtConnectByPINCode) | Directive | CLOVA가 PIN 코드를 요청한 블루투스 기기와 연결하도록 클라이언트에 지시합니다.                        |
| [`BtDelete`](#BtDelete)                   | Directive | CLOVA가 블루투스 페어링 목록에서 특정 기기를 제거하도록 클라이언트에 지시합니다.                        |
| [`BtDisconnect`](#BtDisconnect)           | Directive | CLOVA가 특정 블루투스 기기와 연결을 해제하도록 클라이언트에 지시합니다.                               |
| [`BtPlay`](#BtPlay)                       | Directive | CLOVA가 연결된 블루투스 기기를 통해 오디오 콘텐츠를 재생하도록 클라이언트에 지시합니다.                          |
...
| [`BtRescan`](#BtRescan)                   | Directive | CLOVA가 블루투스 기기를 재탐지(rescan)하도록 클라이언트에 지시합니다.                               |
| [`BtStartPairing`](#BtStartPairing)       | Directive | CLOVA가 블루투스 페어링을 시작하도록 클라이언트에 지시합니다.                                       |
| [`BtStopPairing`](#BtStopPairing)         | Directive | CLOVA가 블루투스 페어링을 중지하도록 클라이언트에 지시합니다.                                       |
| [`Decrease`](#Decrease)                   | Directive | CLOVA가 스피커 볼륨 또는 화면 밝기를 기본 단위만큼 줄이도록 클라이언트에 지시합니다.                     |
| [`ExpectReportState`](#ExpectReportState) | Directive | CLOVA가 기기의 현재 상태를 CIC로 보고하도록 클라이언트에 지시합니다.                                 |
| [`Increase`](#Increase)                   | Directive | CLOVA가 스피커 볼륨 또는 화면 밝기를 기본 단위만큼 높이도록 클라이언트에 지시합니다.                     |
| [`LaunchApp`](#LaunchApp)                 | Directive | **(Deprecated)** CLOVA가 특정 앱을 실행하도록 클라이언트에 지시합니다.                           |
| [`Open`](#Open)                           | Directive | CLOVA가 특정 화면을 표시하도록 클라이언트에 지시합니다.                                               |
| [`OpenScreen`](#OpenScreen)               | Directive | **(Deprecated)** CLOVA가 설정 화면을 열도록 클라이언트에 지시합니다.                              |
| [`ReportState`](#ReportState)             | Event     | 클라이언트는 기기의 현재 상태를 CIC로 보고할 때 이 메시지를 사용해야 합니다.                     |
| [`RenderDeviceList`](#RenderDeviceList)   | Directive   | CLOVA가 사용자 계정에 등록된 클라이언트 기기 목록을 표시하도록 클라이언트에 지시합니다.  |
| [`RequestDeviceList`](#RequestDeviceList)             | Event     | 클라이언트는 사용자 계정에 등록된 클라이언트 기기 목록과 각 클라이언트의 식별 정보를 파악하고자 할 때 이 이벤트 메시지를 CIC로 전송합니다. |
...
| [`SetValue`](#SetValue)                   | Directive | CLOVA가 스피커 볼륨 또는 화면 밝기를 지정한 값으로 설정하도록 클라이언트에 지시합니다.                    |
| [`SynchronizeState`](#SynchronizeState)   | Directive | CLOVA가 사용자 계정에 등록된 또 다른 클라이언트 기기의 상태를 업데이트하도록 클라이언트에 지시합니다.         |
| [`TurnOff`](#TurnOff)                     | Directive | CLOVA가 지정한 기능이나 모드를 끄거나 비활성화하도록 클라이언트에 지시합니다.                           |
| [`TurnOn`](#TurnOn)                       | Directive | CLOVA가 지정한 기능을 켜거나 활성화하도록 클라이언트에 지시합니다.                                   |
...
CLOVA가 페어링된 블루투스 기기 중 하나와 연결을 설정하도록 클라이언트에 지시합니다. CIC는 연결할 블루투스 기기를 지정하거나 지정하지 않을 수 있습니다.
...
CLOVA가 PIN 코드를 요청한 블루투스 기기와 연결하도록 클라이언트에 지시합니다. CIC는 이 메시지에 사용자가 입력한 PIN 코드를 전달합니다.
...
CLOVA가 블루투스 페어링 목록에서 특정 기기를 제거하도록 클라이언트에 지시합니다.
...
CLOVA가 연결된 블루투스 기기와 연결을 끊도록 클라이언트에 지시합니다.
...
CLOVA가 연결된 블루투스 기기를 통해 오디오 콘텐츠를 재생하도록 클라이언트에 지시합니다. 사용자가 "블루투스로 음악 재생해줘"와 같은 명령을 했을 때 이 지시 메시지가 전달됩니다. 클라이언트는 다른 기기와 블루투스로 연결될 때 오디오 스트림을 수신해서 출력하는 역할(`"sink"`)이나 오디오 스트림을 송신하는 역할(`"source"`)을 맡게되며, 클라이언트는 각 역할(`role`)에 따라 이 지시 메시지를 처리해야 합니다.
...
CLOVA가 블루투스 기기를 재탐지(rescan)하도록 클라이언트에 지시합니다. 주변 연결 가능한 블루투스 기기 목록을 보여주는 페어링 화면을 표시하거나 사용자가 목록의 갱신을 요청하면 CIC가 클라이언트로 전달합니다.
...
CLOVA가 블루투스 페어링을 시작하도록 클라이언트에 지시합니다.
...
CLOVA가 블루투스 페어링을 중지하도록 클라이언트에 지시합니다.
...
CLOVA가 스피커 볼륨 또는 화면 밝기를 기본 단위만큼 줄이도록 지시하거나, TV 채널을 이전 채널로 변경하도록 클라이언트에 지시합니다.
...
CLOVA가 기기의 현재 상태를 CIC로 보고하도록 클라이언트에 지시합니다. 이 메시지를 받은 클라이언트는 즉시 [`DeviceControl.ReportState`](#ReportState) 이벤트 메시지를 사용하여 기기의 현재 상태를 보고해야 하며, 이후에는 `durationInSeconds` 필드에 지정된 시간 동안 `intervalInSeconds` 필드에 지정된 주기로 기기의 현재 상태를 보고해야 합니다.
...
CLOVA가 스피커 볼륨 또는 화면 밝기를 기본 단위만큼 높이도록 지시하거나, TV 채널을 다음 채널로 변경하도록 클라이언트에 지시합니다.
...
**(Deprecated)** CLOVA가 특정 앱을 실행하도록 클라이언트에 지시합니다. 앱을 지정하는 값으로 앱의 custom URI scheme나 중계 페이지 URI 또는 앱 이름이 전달됩니다.
...
CLOVA가 특정 화면을 표시하도록 클라이언트에 지시합니다.
...
**(Deprecated)** CLOVA가 설정 화면을 열도록 클라이언트에 지시합니다.
...
CLOVA가 사용자 계정에 등록된 클라이언트 기기 목록을 표시하도록 클라이언트에 지시합니다.
...
클라이언트는 사용자 계정에 등록된 클라이언트 기기 목록과 각 클라이언트의 식별 정보를 파악하고자 할 때 이 이벤트 메시지를 CIC로 전송합니다.
...
CLOVA가 스피커 볼륨 또는 화면 밝기를 지정한 값으로 설정하거나 특정 TV 채널로 변경하도록 클라이언트에 지시합니다.
...
CLOVA가 지정한 기능이나 모드를 끄거나 비활성화하도록 클라이언트에 지시합니다. 예를 들면, 이 지시 메시지를 통해 클라이언트 기기의 블루투스를 끄도록 할 수 있습니다.
...
CLOVA가 지정한 기능을 켜거나 활성화하도록 클라이언트에 지시합니다. 예를 들면, 이 지시 메시지를 통해 클라이언트 기기의 블루투스를 켜도록 할 수 있습니다.
