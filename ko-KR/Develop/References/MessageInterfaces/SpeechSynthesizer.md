...
| [`Speak`](#Speak)                     | Directive | CLOVA가 합성된 TTS를 스피커로 출력하도록 클라이언트에 지시합니다.                                        |
...
CLOVA가 합성된 TTS를 스피커로 출력하도록 클라이언트에 지시합니다. 클라이언트는 하나의 요청에 대한 응답으로 복수의 `SpeechSynthesizer.Speak` 지시 메시지를 전달받을 수 있습니다. 따라서, 클라이언트는 메시지를 수신한 순서대로 TTS를 재생해야 합니다. TTS는 [multipart 메시지](/Develop/References/CIC_API.md#MultipartMessage)로 전달될 수도 있고 오디오 스트리밍 주소 형태로 전달될 수도 있습니다.
